...
## set current week 
reporting_week <- yearweek("2021 W08", week_start = 7)
...
From the start of the outbreak up until `r reporting_week` there were a total of `r nrow(filter(linelist_cleaned, epiweek <= reporting_week))` cases, of which `r nrow(filter(linelist_cleaned, epiweek == reporting_week)` were identified in `r reporting_week`. There were `r fmt_count(filter(linelist_cleaned, epiweek <= reporting_week), sex == "Female")` females and `r fmt_count(filter(linelist_cleaned, epiweek <= reporting_week), sex == "Male")` males. The most affected age group was `r tab_linelist(filter(linelist_cleaned, epiweek <= reporting_week), age_group) %>% slice(which.max(n)) %>% pull(value)` years.  There have been a total of `r fmt_count(filter(linelist_cleaned, epiweek <= reporting_week), exit_status == 2)` deaths and `r fmt_count(filter(linelist_cleaned, epiweek <= reporting_week), exit_status == 4 | exit_status == "Admission")` admissions and `r fmt_count(linelist_cleaned, cholera_rdt_result == "+")` Cholera RDT positive cases. The most affected camp was `r tab_linelist(linelist_cleaned, patient_origin) %>% slice(which.max(n)) %>% pull(value)`.
